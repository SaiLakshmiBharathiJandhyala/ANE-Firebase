	...
	public class FirebaseAuthTests
	{
		...
		public function setup(e:Event=null):void
		{
			try
			{
				...
				FacebookAPI.init( Config.distriqtApplicationKey );
				...
				if (FacebookAPI.isSupported)
				{
					...
					FacebookAPI.service.initialiseApp( Config.facebookAppId, Config.urlSchemeSuffix );
					
					log("sdk version:            " + FacebookAPI.service.getSDKVersion() );
					log("isFacebookAppInstalled: " + FacebookAPI.service.isFacebookAppInstalled());
					log("isSessionOpen:          " + FacebookAPI.service.isSessionOpen());
				}
		...
		public function facebookSignIn():void
		{
			log( "Facebook signIn()  " );
			
			if (FacebookAPI.service.isSessionOpen())
			{
				log( "Facebook signIn(): Session already open" );
				_accessToken = FacebookAPI.service.getAccessToken().token;
			}
			else
			{
				...
				log( "Facebook signIn(): Creating session " + success );
			}
		}
		
		private function fb_sessionOpenedHandler( event:FacebookAPISessionEvent ):void
		{
			log("FacebookAPISessionEvent::SESSION_OPENED");
			log( "accessToken="+event.accessToken.token );
			
			_accessToken = event.accessToken.token;
		}
		
		private function fb_sessionInfoHandler( event:FacebookAPISessionEvent ):void
		{
			log("FacebookAPISessionEvent::SESSION_INFO");
			log( "profile: " + event.profile.name );
		}
		
		private function fb_sessionOpenErrorHandler( event:FacebookAPISessionEvent ):void
		{
			log("FacebookAPISessionEvent::SESSION_OPEN_ERROR");
			log( "error=["+event.errorCode+"]: "+ event.errorMessage );
		}
		
		private function fb_sessionOpenCancelledHandler( event:FacebookAPISessionEvent ):void
		{
			log("FacebookAPISessionEvent::SESSION_OPEN_CANCELLED");
		}
		
		private function fb_sessionOpenDisabledHandler( event:FacebookAPISessionEvent ):void
		{
			log("FacebookAPISessionEvent::SESSION_OPEN_DISABLED");
		}
		...
		public function signIn():void
		{
			...
			var credential:AuthCredential = FacebookAuthProvider.getCredential( _accessToken );
